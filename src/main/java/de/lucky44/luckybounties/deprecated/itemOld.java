...
public class itemOld {
    ...
    //Enchantments for books and tools
    ...
    //Extra space for various things
    public String[] extras;

    //Item container
    public itemOld[] content;
    public transient ItemStack converted = null;

    public itemOld(ItemStack iS){
        ...
        flags = iM.getItemFlags().toArray(new ItemFlag[0]);
        ...
        if(m.equals(Material.ENCHANTED_BOOK)){
            ...
            ArrayList<itemOld> newItems = new ArrayList<>();
            ...
            for(ItemStack I : sB.getInventory().getContents()){
                if(I != null){
                    newItems.add(new itemOld(I));
                }
            }

            content = newItems.toArray(new itemOld[0]);
        }
        else{
    ...
    private ItemStack toItem(){
        ...
        ItemStack i = new ItemStack(Objects.requireNonNull(Material.getMaterial(_Material)));
        ...
        if(i.getType().equals(Material.ENCHANTED_BOOK)){
            ...
            for (int idx = 0; idx < enchantments.length; idx++) {
                eSM.addStoredEnchant(Enchantment.getByName(enchantments[idx]), enchantmentLevels[idx], false);
            }
