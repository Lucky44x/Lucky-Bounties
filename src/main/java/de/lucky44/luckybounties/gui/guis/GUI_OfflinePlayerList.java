...
public class GUI_OfflinePlayerList extends ChestGUI {
    ...
    public GUI_OfflinePlayerList(int page, OfflinePlayer[] players){
    ...
    public void onClick(int slot, ItemStack item) {
        ...
        if(slot == pageRows*9-1){
            ...
            GUI_OfflinePlayerList list = new GUI_OfflinePlayerList(page+1, players);
            list.open(user);
        }
        ...
        if(slot % 2 != 0) {
            ...
            if(playerIndex  + offset < players.length){
                OfflinePlayer target = players[playerIndex + offset];
                GUI_BountiesList bountiesList = new GUI_BountiesList((Player)target, 0);
